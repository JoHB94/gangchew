...
export default function MyPostListBox(props) {
  //*****************************state********************************************* */
    ...
    const requestData = async () => {
      try {
        const response = await axios({
          url: `${LOCAL_IP}/studentrequest/all`, // 수강자 전체 게시글 요청
          method: "GET",
          headers: {
        ...
        for (let i = 0; i < response.data.result.length; i++) {
          if (response.data.result[i].writer === props.myUsername) {
            // 상위 컴포넌트에서 api요청한 usernickname과 대조해 일치하는 게시물만 출력될 수 있도록 데이터를 필터링
