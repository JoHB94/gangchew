...
<<<<<<< HEAD
  ...
  const fundingStartClick = () => {
    const userRes = window.confirm(
      "펀딩이 시작되면 수정할 수 없습니다. 정말로 시작하시겠습니까?"
    );
    if (userRes) {
      axiosInstance
        .post(localIP + "funding/update?state=IN_PROGRESS")
        .then((res) => {
          console.log(res);
          alert("펀딩이 시작되었습니다.");
        })
...
>>>>>>> 141bbb26011dda58fe81547aeee95673a2ac0681
  ...
  const fundingLikeClick = () => {
    axiosInstance
      .get(localIP + `funding/toggle-like?id=${data.funding.id}`)
      .then((res) => {
        console.log(res);
        setLiked(!liked);
      })
    ...
    <div>
      <div id="f_container">
        ...
        <div id="f_center">
          <div id="f_headerarea"></div>
          <div id="f_headerarea"></div>
          <h1 id="f_title">{data.funding.title}</h1>
          {/**센터 */}
          <div id="f_bigBox">
            <div id="f_contentBox">
              {/**내용 컨테이너 */}
            ...
            <div id="f_payBox">
              ...
              <div className="fixed_element">
                ...
                <div id="f_like_visit">
                  <AiFillEye />
                  &nbsp; {data.funding.viewCount} &nbsp;
                  <AiFillHeart />
                  &nbsp; {data.funding.likeCount}
                </div>
                <p id="f_infoLetter">
                  카테고리 &nbsp;
                  <BiSolidRightArrow />
                  &nbsp;{data.fundingCategory.categoryName}
                </p>
                <hr />

                <ul>
                  <li id="f_info_li" className="f_Dday">
                    <h3>{diffDayValue}</h3>
                  </li>
                  <li id="f_info_li">마감일 {data.deadline}</li>
                  <li id="f_info_li">금액 {data.funding.amount}원</li>
                  <li id="f_info_li">
                    최대 모집 인원 {data.funding.maxParticipants}명
                  </li>
                  <li id="f_info_li">
                    현재 남은 인원 명{/**funding.max_participants - 참여인원 */}
                  </li>
                  {isOffline ? (
                    <div>
                      <li id="f_info_li">
                  ...
                  <li id="f_info_li">
                    <h2 style={{ color: "#fa6363" }}>
                      달성률 {data.achievementrate}%{/**목표인원/참여인원 */}{" "}
          ...
          <div id="f_writer_boxes">
            ...
            <div id="f_writer_msg">
              <h3 id="f_msg_title" style={{ paddingLeft: "15px" }}>
                펀딩 시작하기 &nbsp;{" "}
                <FiAlertTriangle style={{ color: "red" }} size={28} />
              </h3>
              <div id="f_msg_content" style={{ paddingLeft: "15px" }}>
                펀딩 시작하기 버튼을 클릭시 펀딩이 시작됩니다.
                <br />
                펀딩이 시작되면 내용 수정이 불가능합니다.
              </div>
            </div>
          </div>
          <div id="f_height150"></div>
